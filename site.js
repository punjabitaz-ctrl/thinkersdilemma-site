...
(function () {
  ...
  /* ---------- Automated Archive Fetcher (RSS to JSON) ------------ */
  function updateArchive() {
    var containers = document.querySelectorAll('[data-render="home-archive"]');
    if (containers.length === 0) return;

    // We fetch the JSON file generated by your GitHub Action
    fetch('articles.json')
      .then(function(res) { return res.json(); })
      .then(function(data) {
        // Handle both raw arrays and rss2json objects (data.items)
        var posts = data.items || data;
        var html = posts.slice(0, 6).map(function(item) {
          return '<div class="essay-card">' +
                 '  <a href="' + item.link + '">' +
                 '    <h3>' + item.title + '</h3>' +
                 '    <p>' + (item.description || item.contentSnippet || '') + '</p>' +
                 '  </a>' +
                 '</div>';
        }).join('');
  ...
  function init() {
    ...
    updateArchive(); // Automatically fills the archive section
  }
